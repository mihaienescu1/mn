...
class User_need_delivery extends CI_Model
{
	private $__tableName 	= 	'user_need_delivery';
	private $__fields 		= 	array('id', 'user_id', 'need_id', 'content', 'created', 'modified', 'detect_links', 'type', 'has_file');
	
	private $__users_table  = "users";
	
	private $__delivery_comment_table  = "user_need_delivery_comment";
	private $__delivery_like_table  = "user_need_delivery_like";
	...
	{
		$strQuery	=	'SELECT `'.$this->__tableName.'`.`id` AS delivery_id, 
	...
	public function getByNeedIdWithInformation($need_id, $user_id ,$limit, $offset)
	{
		$strQuery	=	'SELECT `'.$this->__tableName.'`.`id` AS delivery_id, 
								...
								UNIX_TIMESTAMP(`'.$this->__tableName.'`.created) AS created_unix ,
								`'.$this->__tableName.'`.* ,
								(SELECT COUNT(`'.$this->__delivery_like_table.'`.`user_id`) FROM `'.$this->__delivery_like_table.'`
									WHERE `'.$this->__delivery_like_table.'`.`delivery_id` = `'.$this->__tableName.'`.`id`
								) as like_count,
									
								(SELECT COUNT(`'.$this->__delivery_comment_table.'`.`id`) FROM `'.$this->__delivery_comment_table.'`
									WHERE `'.$this->__delivery_comment_table.'`.`delivery_id` = `'.$this->__tableName.'`.`id`) as comment_count,
								
								IF(
								   (SELECT COUNT(`'.$this->__delivery_like_table.'`.`user_id`) 
								           FROM `'.$this->__delivery_like_table.'`
									       WHERE `'.$this->__delivery_like_table.'`.`delivery_id` = `'.$this->__tableName.'`.`id`
										         AND `'.$this->__delivery_like_table.'`.`user_id` = '.(int)$user_id.'
									) > 0, 
