...
class AJAX_Activities extends CI_Controller
{
	...
	public function __construct()
	{
		...
		self::$CI->load->model('Activities');
		self::$CI->load->model('Activities_likes');
		self::$CI->load->model('Activities_comments');
		self::$CI->load->model('Activities_comments_likes');
	}
	...
	public function add()
	{
		...
		$addPost	=	self::$CI->Activities->add($need, $user, $content, $hasLinks, $type, $filepath, $meta_description);
		...
		if($addPost) {
			...
			$uinfo = self::$CI->Users_model->getById($user);
			
			switch($type) {
				case ('SHARE_LINK' || 'SOCIAL_SHARE') :
					self::$CI->Need_status_messages->add(array(
						'need_id' 		=> 	$need,
						'action_text'	=>	sprintf(DEFAULT_NEED_SOCIAL_SHARE_STATUS_MESSAGE_FORMAT, $uinfo['id'], $uinfo['firstname'], $uinfo['lastname'])
					));
				break;
				...
				case 'SOCIAL_LIKE' : 
					self::$CI->Need_status_messages->add(array(
						'need_id' 		=> 	$need,
						'action_text'	=>	sprintf(DEFAULT_NEED_SOCIAL_LIKE_STATUS_MESSAGE_FORMAT, $uinfo['id'], $uinfo['firstname'], $uinfo['lastname'])
					));
				break;
			}
	...
	public function delete()
	{
		...
		if(isset($_POST['meta']) && !empty($_POST['meta']))
		{
			...
			$act		=	self::$CI->Activities->getMyMetaRow($meta, $need, $user);
		...
		$response	=	array('status'	=>	'', 'activity_id'	=>	$activity, 'need_id'	=>	$need);
		
		if(!is_null($activity))
		{
			$deletePost		=	self::$CI->Activities->del($activity, $user, $need);
			$deleteComments	=	self::$CI->Activities_comments->del($activity, false);
			$deleteLikes	=	self::$CI->Activities_likes->remove($activity, false);
		}
	...
	public function getByNeed()
	{
		...
		$limit				=		$_POST['limit'];
		$offset				=		$_POST['offset'];
		$comments			=		self::$CI->Activities->getByNeedIdWithInformation($need, $user, $limit, $offset);
		$current			=		0;
		$html				=		"";
		$currentActivity = 0;
		...
		{ 
			...
			$img_url			=	!empty($c['detect_links']) ? 'http://immediatenet.com/t/m?Size=1024x768&URL='.$c['detect_links'] : '../img/shared_photo_sample.jpg';
			...
			{
				if($current == 0) {
					$html .='<section id="section-need-comments-'.$c['activity_id'].'" class="comments-list first-line">';
				} else {
					$html .='<section id="section-need-comments-'.$c['activity_id'].'" class="comments-list">';
				}				
				$html .='<article>';
					...
					$html .='<div class="info">';
						...
						$html .='<div class="actions">';
							...
							if($is_liked){
								$html .='<a href="javascript:void(0);" class="add_action unlike first" id="'.$c['activity_id'].'">UnLike</a>';
							}	
							else{
								$html .='<a href="javascript:void(0);" class="add_action like first" id="'.$c['activity_id'].'">Like</a>';
							}
							$html .='<a href="javascript:void(0);" class="add_action comment" id="'.$c['activity_id'].'">Comment</a>';
							$html .='<a href="javascript:void(0);" class="add_action share" id="'.$c['activity_id'].'">Share</a>';
							if($c['act_user'] == $user)
							{
								$html .='<a href="javascript:void(0);" class="add_action delete" id="'.$c['activity_id'].'">Delete</a>';
							}
						$html .='</div>';
							if($is_liked)
							{
								$html .='<a class="icon comments like on unlinke_thumbs" id="'.$c['activity_id'].'"><span></span>Like</a>';
							}
							else
							{
								$html .='<a class="icon comments like" id="'.$c['activity_id'].'"><span></span>Like</a>';
							}
							$html .='<a class="icon comments comment" id="'.$c['activity_id'].'"><span></span>Comment</a>';
						$html .='</div>';
						$html .='<div style="clear:both; height:4px;"></div>';
								if($likes_count > 0 || $comments_count > 0)
								{
									$html .='<section class="comments-like" id="'.$c['activity_id'].'">';
										$html .='<div class="left">';
											if($likes_count > 0)
											{
												$html .='<a href="javascript:void(0);" class="icon view-like cm" id="'.$c['activity_id'].'">';
													$html .='<span></span>';
													$html .='<strong class="likes_count_'.$c['activity_id'].'">' . $likes_count . ' People</strong> <b>like this</b>';
												$html .='</a>';
											}
											else
											{
												$html .='<a href="javascript:void(0);" class="icon view-like cm dontShow" id="'.$c['activity_id'].'">';
													$html .='<span></span>';
													$html .='<strong class="likes_count_'.$c['activity_id'].'">' . $likes_count . ' People</strong> <b>like this</b>';
												$html .='</a>';
											}
											...
											if($comments_count > 0)
											{
												...
												$html .= '<a href="javascript:void(0);" class="icon view-comments cm" id="'.$c['activity_id'].'">';
													$html .= '<span></span>';
													$html .= '<strong class="comments_count_'.$c['activity_id'].'">' . $text . '</strong>';
												$html .= '</a>';
											}
											else
											{
												
												$html .= '<a href="javascript:void(0);" class="icon view-comments cm dontShow" id="'.$c['activity_id'].'">';
													$html .= '<span></span>';
													$html .= '<strong class="comments_count_'.$c['activity_id'].'"></strong>';
												$html .= '</a>';
											}
										$html .='</div>';
								...
								if($likes_count == 0 && $comments_count == 0)
								{
									$html .='<section class="comments-like dontShow" id="'.$c['activity_id'].'">';
									$html .='<div class="left">';
								...
								$html .='<div class="activity_comments" id="ac_' . $c['activity_id'] . '">';
								$html .='<a href="javascript:void(0);" class="show_more_sub" id="'.$c['activity_id'].'" limit="">show previous comments...</a>';
								$html .='</div>';
							$html .='<div class="comments_add_wrapper" id="cax_' . $c['activity_id'] . '">';
								$html .='<input type="text" class="comments_add_box" default="Add comment..." value="" id="' . $c['activity_id'] . '">';
							$html .='</div>';
					$html .= '</div>';
				$html .='</article>';
			}
			...
			
			{
				...
				$html.='<div id="publish_info_'.$c['activity_id'].'">';
	...
	public function like()
	{
		$user_id	=	$_POST['uid'];
		$act_id		=	$_POST['actid'];
		$result		=	array('status'	=>	'', 'activity_id' => $act_id);
		
		$result['status']	=	self::$CI->Activities_likes->add($act_id, $user_id);
		$result['total']	=	self::$CI->Activities_likes->getCount($act_id);
	...
	public function unlike()
	{
		...
		$result['status']	=	self::$CI->Activities_likes->remove($act_id, $user_id);
		$result['total']	=	self::$CI->Activities_likes->getCount($act_id);
	...
	public function commentLike()
	{
		...
		$result['status']	=	self::$CI->Activities_comments_likes->add($user_id, $comment_id);
	...
	public function commentUnlike()
	{
		...
		$result['status']	=	self::$CI->Activities_comments_likes->remove($user_id, $comment_id);
	...
	public function addComment()
	{
		...
		$result['status']				=	self::$CI->Activities_comments->add($act_id, $user_id, $ctext);
		$result['total_comments']		=	self::$CI->Activities_comments->getCount($act_id);
		$result['total_likes']			=	self::$CI->Activities_likes->getCount($act_id);	
	...
	public function delComment()
	{
		...
		$result		=	array('status'	=>	'', 'comment_id' => $comment, 'activity_id'	=> $activity);
		
		$result['status']		=	self::$CI->Activities_comments->del($user_id, $comment);
		$result['total']		=	self::$CI->Activities_comments->getCount($activity);
		$result['total_likes']	=	self::$CI->Activities_likes->getCount($activity);
	...
	{	
		...
		$comments	=	self::$CI->Activities_comments->get($act_id, $limit, $offset, $order);
		...
		foreach($comments['results'] as $c)
		{
			$liked	=	self::$CI->Activities_comments_likes->isLiked($c['id'], $user_id);
	...
	public function getLikes()
	{
		...
		$get_likes	=	self::$CI->Activities_likes->get($act_id);
		$get_count	=	self::$CI->Activities_likes->getCount($act_id);
	...
	public function likeCommentsNumber()
	{
		...
		$result	=	array(
						  ...
						  'activity_id' => 		$act_id,
						  'user_id'		=>		$user_id,
						  'comments'	=>		self::$CI->Activities_comments->getCount($act_id),
						  'likes'		=>		self::$CI->Activities_likes->getCount($act_id)
					);
