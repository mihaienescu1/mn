...
class AJAX_UserProvision extends CI_Controller
{
	...
	public function __construct() {
		...
		self::$CI->load->model('User_provision');
		self::$CI->load->model('User_provision_like');
		self::$CI->load->model('User_provision_comment');
		self::$CI->load->model('User_provision_comment_like');
	}
	
	public function add() {
		$response		=	array('status'	=>	'');
		
		$type = (isset($_POST['activity_type']) && !empty($_POST['activity_type'])) ? trim($_POST['activity_type']) : 'COMMENT';
		$need				=	$_POST['need_id'];
		$user				=	$_POST['user_id'];
		$content			=	$_POST['content'];
		$updateStatus		=	(isset($_POST['update_status']) && !empty($_POST['update_status'])) ? 1 : 0;
		$hasLinks			=	null;
		$filepath			=	null;
		$meta_description	=	(isset($_POST['meta']) && !empty($_POST['meta'])) ? trim($_POST['meta']) : null;
		
		$regex_url	= 	"/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/";
		preg_match($regex_url, $content, $urls);
		
		if(count($urls)
			&& is_null($meta_description)) {
		...
		$addPost	=	self::$CI->User_provision->add($need, $user, $content, $hasLinks, $type, $filepath, $meta_description);
		...
		$response['status'] = ($addPost) ? 'POST_ADDED_OK' : 'PROCESSING_ERROR';
	...
	public function delete() {
		...
		if(isset($_POST['meta']) && !empty($_POST['meta']))
		{
			...
			$act		=	self::$CI->User_provision->getMyMetaRow($meta, $need, $user);
			
			if( isset($act[0]) )	$activity	=	$act[0]['id'];
		}
		else
		{
			$activity	=	$_POST['actid'];
		}
		
		$response	=	array('status'	=>	'', 'provision_id'	=>	$activity, 'need_id'	=>	$need);
		
		if(!is_null($activity))
		{
			$deletePost		=	self::$CI->User_provision->del($activity, $user, $need);
			$deleteComments	=	self::$CI->User_provision_comment->del($activity, false);
			$deleteLikes	=	self::$CI->User_provision_like->remove($activity, false);
		}
		
		$response['status'] = ($deletePost && $deleteComments && $deleteLikes) ? 'POST_DELETED_OK' : 'PROCESSING_ERROR';
		
		return $response;
	}
	
	public function getByNeed() {
		$response		=	array('status'	=>	'', 'activity_html' => '', 'activity_json' =>	'');
	
		$need		=	$_POST['need_id'];
		$user		=	$_POST['user_id'];
		$limit		=	$_POST['limit'];
		$offset		=	$_POST['offset'];
		$comments	=	self::$CI->User_provision->getByNeedIdWithInformation($need, $user, $limit, $offset);
		$current	=	0;
		$html		=	"";
		...
		foreach ($comments['results'] as $c) { 
			...
			{
				if($current == 0) {
					$html .='<section id="section-need-comments-'.$c['provision_id'].'" class="comments-list first-line">';
				} else {
					$html .='<section id="section-need-comments-'.$c['provision_id'].'" class="comments-list">';
				}				
				$html .='<article>';
					...
					$html .='<div class="info">';
						...
						$html .='<div class="actions">';
							...
							if($is_liked){
								$html .='<a href="javascript:void(0);" class="add_action unlike first" id="'.$c['provision_id'].'">UnLike</a>';
							}	
							else{
								$html .='<a href="javascript:void(0);" class="add_action like first" id="'.$c['provision_id'].'">Like</a>';
							}
							$html .='<a href="javascript:void(0);" class="add_action comment" id="'.$c['provision_id'].'">Comment</a>';
							$html .='<a href="javascript:void(0);" class="add_action share" id="'.$c['provision_id'].'">Share</a>';
							if($c['act_user'] == $user)
							{
								$html .='<a href="javascript:void(0);" class="add_action delete" id="'.$c['provision_id'].'">Delete</a>';
							}
						$html .='</div>';
							if($is_liked)
							{
								$html .='<a class="icon comments like on unlinke_thumbs" id="'.$c['provision_id'].'"><span></span>Like</a>';
							}
							else
							{
								$html .='<a class="icon comments like" id="'.$c['provision_id'].'"><span></span>Like</a>';
							}
							$html .='<a class="icon comments comment" id="'.$c['provision_id'].'"><span></span>Comment</a>';
						$html .='</div>';
						$html .='<div style="clear:both; height:4px;"></div>';
								if($likes_count > 0 || $comments_count > 0)
								{
									$html .='<section class="comments-like" id="'.$c['provision_id'].'">';
										$html .='<div class="left">';
											if($likes_count > 0)
											{
												$html .='<a href="javascript:void(0);" class="icon view-like cm" id="'.$c['provision_id'].'">';
													$html .='<span></span>';
													$html .='<strong class="likes_count_'.$c['provision_id'].'">' . $likes_count . ' People</strong> <b>like this</b>';
												$html .='</a>';
											}
											else
											{
												$html .='<a href="javascript:void(0);" class="icon view-like cm dontShow" id="'.$c['provision_id'].'">';
													$html .='<span></span>';
													$html .='<strong class="likes_count_'.$c['provision_id'].'">' . $likes_count . ' People</strong> <b>like this</b>';
												$html .='</a>';
											}
											...
											if($comments_count > 0)
											{
												...
												$html .= '<a href="javascript:void(0);" class="icon view-comments cm" id="'.$c['provision_id'].'">';
													$html .= '<span></span>';
													$html .= '<strong class="comments_count_'.$c['provision_id'].'">' . $text . '</strong>';
												$html .= '</a>';
											}
											else
											{
												
												$html .= '<a href="javascript:void(0);" class="icon view-comments cm dontShow" id="'.$c['provision_id'].'">';
													$html .= '<span></span>';
													$html .= '<strong class="comments_count_'.$c['provision_id'].'"></strong>';
												$html .= '</a>';
											}
										$html .='</div>';
								...
								if($likes_count == 0 && $comments_count == 0)
								{
									$html .='<section class="comments-like dontShow" id="'.$c['provision_id'].'">';
									$html .='<div class="left">';
								...
								$html .='<div class="provision_comments" id="ac_' . $c['provision_id'] . '">';
								$html .='<a href="javascript:void(0);" class="show_more_sub" id="'.$c['provision_id'].'" limit="">show previous comments...</a>';
								$html .='</div>';
							$html .='<div class="comments_add_wrapper" id="cax_' . $c['provision_id'] . '">';
								$html .='<input type="text" class="comments_add_box" default="Add comment..." value="" id="' . $c['provision_id'] . '">';
							$html .='</div>';
					$html .= '</div>';
				$html .='</article>';
			} else {
				...
				$html.='<div id="publish_info_'.$c['provision_id'].'">';
		...
		$response['activity_html']	=	$html; unset($html);
		$response['activity_json']	=	json_encode($comments);
		$response['status']			=	'OK';
	...
	public function like() {
		$user_id	=	$_POST['uid'];
		$act_id		=	$_POST['actid'];
		$result		=	array('status'	=>	'', 'provision_id' => $act_id);
		
		$result['status']	=	self::$CI->User_provision_like->add($act_id, $user_id);
		$result['total']	=	self::$CI->User_provision_like->getCount($act_id);
	...
	public function unlike() {
		...
		$result['status']	=	self::$CI->User_provision_like->remove($act_id, $user_id);
		$result['total']	=	self::$CI->User_provision_like->getCount($act_id);
	...
	public function commentLike() {
		...
		$result['status']	=	self::$CI->User_provision_comment_like->add($user_id, $comment_id);
	...
	public function commentUnlike() {
		...
		$result['status']	=	self::$CI->User_provision_comment_like->remove($user_id, $comment_id);
	...
	public function addComment() {
		...
		$result['status']				=	self::$CI->User_provision_comment->add($act_id, $user_id, $ctext);
		$result['total_comments']		=	self::$CI->User_provision_comment->getCount($act_id);
		$result['total_likes']			=	self::$CI->User_provision_like->getCount($act_id);	
	...
	public function delComment() {
		...
		$result		=	array('status'	=>	'', 'comment_id' => $comment, 'provision_id'	=> $activity);
		
		$result['status']		=	self::$CI->User_provision_comment->del($user_id, $comment);
		$result['total']		=	self::$CI->User_provision_comment->getCount($activity);
		$result['total_likes']	=	self::$CI->User_provision_like->getCount($activity);
	...
	public function getComments() {	
		...
		$comments	=	self::$CI->User_provision_comment->get($act_id, $limit, $offset, $order);
		...
		foreach($comments['results'] as $c)
		{
			$liked	=	self::$CI->User_provision_comment_like->isLiked($c['id'], $user_id);
			...
			$html .='<div class="line" id="'.$c['id'].'">';
					...
					$html .='<div class="user_line_content" id="'.$c['id'].'">';
						...
						$html .= '<p class="timestamp"><small>' . self::$CI->utils->time_since($c['comment_date_unix']) . ' ago</small>';	
							...
							{
								$html .= '<a href="javascript:void(0);" class="sub_comments_delete" rel="'.$c['id'].'" activity="'.$act_id.'">Delete</a>';
							}
						$html .= '</p>';
					$html .='</div>';
			$html .='</div>';
	...
	public function getLikes() {
		...
		$get_likes	=	self::$CI->User_provision_like->get($act_id);
		$get_count	=	self::$CI->User_provision_like->getCount($act_id);
	...
	public function likeCommentsNumber() {
		...
		$result	=	array(
						  ...
						  'provision_id' => 		$act_id,
						  'user_id'		=>		$user_id,
						  'comments'	=>		self::$CI->User_provision_comment->getCount($act_id),
						  'likes'		=>		self::$CI->User_provision_like->getCount($act_id)
					);
	...
	public function getProvisionHeader() {
